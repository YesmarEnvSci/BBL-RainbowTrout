...
opts_chunk$set(fig.width=5, fig.height=4, fig.path='figure/',
...
Fyke-net traps and hoop traps were used to sample Black Bear Lake. Rainbow trout (Oncorhynchus mykiss) captured in the gear were marked to facilitate a mark-recapture experiment to estimate trout abundance. An estimated `r round((N),0)` (SE = `r round((se), 2)`) rainbow trout r >= 180 mm FL were present in 2002.
...
```{r plot1_SEAK, fig.width=5.5, fig.height=5.5, fig.cap="Map of Southeast Alaska, showing location of Black Bear Lake on Prince of Wales Island."}
library(maps)
library(mapdata)

aa <- rep(1,8); ab <- c(1,1,1,1,2,2,2,2)
...
```{r plot3_LenghFreq, fig.cap="Length-frequency distribution of rainbow trout in Black Bear lake, July 2002."}
rbt <- subset(rbt, Length_mm & Recapture == "n")
p4 <- hist(rbt$Length_mm, breaks = 40, 
     xlim = c(min(rbt$Length_mm), max(rbt$Length_mm)), 
     freq = FALSE, 
     col = 'lightblue',
     main = 'Trout Lenght Distribution',
     xlab = 'Length')
lines(density(rbt$Length_mm, bw=10), col = 'red', lwd = 2)
box()
```


```{r plot4_LogLW, fig.cap="Rainbow Trout Natural Log Length (mm) and Weight (g)"} 
library(dplyr)
logLW <- rbt %.% select(Length_mm, Weight_g)%.% mutate(Length_mm=log(Length_mm), Weight_g=log(Weight_g))
logLW <- na.omit(logLW)

scatter <-function(logLW, Length_mm, Weight_g, RBT) {
  require(ggplot2)
  p <- ggplot(data = logLW, aes_string(x = Length_mm, y = Weight_g))
  p <- p + geom_point(color = "Darkgreen") 
  p <- p + stat_smooth(color = "red")
  p <- p + xlab("LN Length (mm)")+ ylab("LN Weight (g)")
  p <- p + theme_bw()
  p <- p + labs (title = RBT)
  print(p)
}
scatter(logLW, "Length_mm", "Weight_g", "Rainbow Trout")
```

```{r subdata, echo=FALSE}
# Year 1 fish removed
subLW <- subset(logLW, Length_mm > 4)
```

To better understand the relationship between fish lenght and weight, we transormed the data for a linear comparison (Figure 4, Table 2).  The length-weight relationship was $Log Weight = 2.85 Log Length - 10.64$, and the estimated correlation between weight and length was `r round(cor(subLW$Weight_g, subLW$Length_mm),3)`.  Since the residuals plots are horizontal (not curved or sloped), equal width, and uniformely scattered along the horizontal axis, we concluded that the regression model is a good fit.  The four age-1 fish (49 to 54 mm) from the second sampling event were not included in the length-weight analysis because they skew the correlation for larger fish.


Table 2.  Linear Model.

```{r table2_LM, echo=FALSE, results='asis'}
lmfit <- lm(subLW$Weight_g ~ subLW$Length_mm)
kable(summary(lmfit)$coef, digits = 2)
```

\pagebreak

```{r Residuals_Plots, echo=FALSE, fig.width=3.5, fig.height=3, fig.cap="Linear fit model residuals plots."}
plot(lmfit)
par(mfrow=c(2,2))
xlab="Weight~Length"
```
...
```{r Length n Weight, include=FALSE}
# Summary Table of Fish Length (mm)
LW <- rbt %.% select(Length_mm, Weight_g)
LW <- na.omit(LW)
summary(LW$Length_mm)
# Summary Table of Fish Weight (g)
summary(LW$Weight_g)
```
...
During the marking event a total of `r round((N),0)` rainbow trout between 180 and 410 mm FL were marked and released alive. During the recapture event `r c` rainbow trout between 46 and 409 mm FL were examined, of which `r r` fish had been marked during the marking event. There was no tag loss detected during the experiment.
...
```{r plot6_CumDist,fig.cap="Cumulative distribution of lenght of rainbow trout new capture enent 1, and new capture event 2." }
rbt180 <- subset(rbt, Length_mm > 180 & Recapture == "n")

df <- data.frame(Length = rbt180$Length_mm, Event = factor(rbt180$CaptureEvent))
df <- na.omit(df) 

p6 <- ggplot(df, aes(Length, group=Event,linetype=Event))
  p6 <- p6 +  stat_ecdf(aes(color=Event))
  p6 <- p6 + labs(title ="Cum Length Dist", x="Length", y = "Cumulative Density")
  p6 <- p6 + theme_bw()
p6
```
...
Abundance of rainbow trout >= 180 mm FL in Black Bear Lake in July 2002 was estimated at `r round((N),0)` (SE = `r round((se), 2)`). The 95% confidence interval (CI) for the estimate was `r round((ci95l), 0)` to `r round((ci95u), 0)`.
...
The condition of most fish caught during this study were between `r quantile(rbt$FultonK, .25)` and `r quantile(rbt$FultonK, .75)` (Figure 7), while the mean condition for the sampled population was `r mean((rbt$FultonK),2)`.   Even though a comparison between years indicates improved condition, the relative condition of the fish compared to other healthy populations is poor. This is primarily a result of very little predation and limited food due to an oligotrophic isolated alpine cirque lake habitat.

```{r plot7_FultonK, fig.cap="Fulton Conditon frequency histogram up to 2.0."}
K <- subset(rbt, FultonK & FultonK < 2.0 & Recapture == "n")
K <-  na.omit(K$FultonK)
pK <- hist(K, breaks = 100, 
    xlim = c(min(K), max(K)), 
    freq = FALSE, 
    col = 'cornsilk3',
...
In 2002, Black Bear Lake had the fifth largest (N = `r round((N), 0)`) rainbow trout population on record during the 7 years of monitoring. This is a conservative estimate since fish < 180 mm were not included in the estimate. From our observations it is felt that the true population value probably lies on the upper side of the population estimate, possibly in the region of 1,000 to 1,100 fish.  In addition, the 95% CI for the 2002 estimate was the smallest for all years of monitoring, indicating that a mark-recapture study is a reasonable method for comparing population differences among years for fish >180 mm.
...
```{r pdf_file, include=FALSE}
# The Pandoc code below converts markdown file to pdf, or Word docx using the pander package.

require(pander)
Pandoc.convert(f="BBLreport.md", format="pdf", options="-s --toc -V geometry:margin=.75in")


# Pandoc.convert(f="BBLreport.md", format="docx", options="-s")
```
